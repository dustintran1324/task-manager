...
public:
	...
	~DLL(); // destructor
	void push(string t, int p, int h, int m); // does what you'd think, with a caveat that if the
	...
	void moveUp(int tn);  // moves task with number tn up one in the list.
	...
	void moveDown(int tn); //moves task with task number tn down one in the list.
	...
	void listDuration(int *th, int *tm, int p);  // gets the total list duration in hours and minutes (passed
	...
	void removeTime(int h, int m); // helper function to remove time from the total time of the list
};
